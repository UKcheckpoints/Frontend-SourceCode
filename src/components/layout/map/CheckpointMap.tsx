...
// Custom marker icons with modern design
...
export default function Component() {
    ...
    const planRoute = async (start: [number, number] = startPoint!, end: [number, number] = endPoint!) => {
        if (!start || !end) {
        ...
        try {
            ...
            const newRoute: Route = {
                id: `route-${Date.now()}`,
                name: `${start[0]},${start[1]} to ${end[0]},${end[1]}`,
    ...
    const handleLocationSearch = async () => {
        try {
            setIsLoading(true)
            const response = await axios.get(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(locationSearchQuery)}`)
            const results: SearchResult[] = response.data.map((result: any) => ({
    ...
    if (!isValid && !isLoadingState) {
        return null // The useJwtValidator hook will handle redirection to the login page
    }
    ...
    return (
        <div className="h-screen flex flex-col bg-gradient-to-b from-sky-100 to-white">
            ...
            <main className="flex-1 flex flex-col lg:flex-row p-4 gap-4">
                <Card className="w-full lg:w-1/3 xl:w-1/4 bg-white shadow-lg z-10 rounded-xl border border-sky-100">
                ...
                <div className="flex-1 relative">
                    <div className="absolute top-4 left-4 right-4 z-[1000] bg-white rounded-lg shadow-md p-2">
                        <div className="flex items-center space-x-2">
                            <Input
                                ...
                                className="flex-grow"
                            />
                            <Button onClick={handleLocationSearch} className="bg-sky-500 hover:bg-sky-600 text-white">
                                <Search className="h-4 w-4" />
                            </Button>
                        </div>
                    ...
                    <MapContainer center={mapCenter} zoom={mapZoom} style={{ height: '100%', width: '100%' }} className="rounded-lg shadow-md">
                        <TileLayer
